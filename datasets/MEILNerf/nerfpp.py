...
class NeRFPPDataset_MEIL(BaseDataset):
    ...
    def split_tasks(self, poses, task_number, task_split_method):
        ...
        if task_split_method == 'random':
            ...
            task_id += ([task_number - 1] *
                        (len(poses) - imgs_per_task * task_number))
        return task_id
        ...
        if split == 'test_traj':
            ...
            if split == 'trainval':
                ...
                if split == 'rep':
                    ...
                    split_name = split
                img_paths = sorted(
                    glob.glob(os.path.join(self.root_dir, split_name,
                                           'rgb/*')))
                poses = sorted(
                    glob.glob(
                        os.path.join(self.root_dir, split_name, 'pose/*.txt')))

            if split == 'train' or split == 'rep':
                # split the training data into 5 tasks
                random.seed(0)
                self.task_ids = self.split_tasks(poses, self.task_number,
                                                 self.task_split_method)
                ...
                print(
                    ...
                    .format(self.id_task_curr, self.rep_size,
                            self.id_train_final))
            else:
