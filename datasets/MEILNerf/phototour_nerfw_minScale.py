...
class PhotoTourDatasetNerfwMS_MEIL(BaseDataset):
    ...
    def split_tasks(self, poses, task_number, task_split_method):
        ...
        if task_split_method == 'random':
            ...
            task_id += ([0] * (len(poses)- imgs_per_task * task_number)) 
        ...
        if split == 'train' :
            ...
            for i, t in enumerate(tqdm(self.id_train_final)):
                ...
                if self.img_wh[t][0] * self.img_wh[t][1] != mask.shape[0]:
                    ...
                    if t in self.id_rep:
                        self.id_rep_MEIL+=list(range(ray_id_curr, ray_id_curr + buf[0].shape[0]))
                        if i % 100 == 0:
                            print("{} in id_rep  --> self.id_rep_MEIL[-10:] = {}/{}, ray_id_curr = {}, buf[0].shape = {}".format(t, self.id_rep_MEIL[-10:], len(self.id_rep_MEIL), ray_id_curr,  buf[0].shape[0]))
                    else:
                        self.id_curr_MEIL+=list(range(ray_id_curr, ray_id_curr + buf[0].shape[0]))
                        if i % 100 == 0:
                            print("{} not in id_rep  --> self.id_curr_MEIL[-10:] = {}/{}, ray_id_curr = {}, buf[0].shape = {}".format(t, self.id_curr_MEIL[-10:], len(self.id_curr_MEIL), ray_id_curr,  buf[0].shape[0]))
                    ray_id_curr+=buf[0].shape[0]
            ...
            for i, t in enumerate(tqdm(self.id_train_final)):
                ...
                if self.img_wh[t][0] * self.img_wh[t][1] != img.shape[0]:
                    print("skip data generation for image {} (meta size = {}, actual size = {})".format(mask_paths[t], self.img_wh[t], img.shape[0])) 
    ...
    def __getitem__(self, idx):
        if self.split.startswith('train'):
            if self.task_curr == 0:
                ...
                # randomly sample a batch of pixels
                ray_idxs = torch.cat((self.id_rep_MEIL[np.random.choice(self.id_rep_MEIL.shape[0], self.batch_size//3)], 
