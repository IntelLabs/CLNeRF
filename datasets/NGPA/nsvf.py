...
from ..color_utils import read_image, read_image_ngpa
...
class NSVFDataset_NGPA(BaseDataset):
    ...
    def read_meta(self, split):
        ...
        if split == 'test_traj': # BlendedMVS and TanksAndTemple
            ...
            t = 0
            for img_id, (img_path, pose) in enumerate(tqdm(zip(img_paths, poses))):
                c2w = np.loadtxt(pose)[:3]
                c2w[:, 3] -= self.shift
                c2w[:, 3] /= 2*self.scale # to bound the scene inside [-0.5, 0.5]
                self.poses += [c2w]

                img = read_image_ngpa(img_path, self.img_wh, split = split, t = t, test_img_gen = True)
                ...
                # print("[test before train]: img = {}/{}/{}".format(img, img.min(), img.max()))

                self.rays += [img]
                self.ts += [t]
                if (img_id + 1) % self.img_per_appearance == 0:
                    t += 1
    ...
    def __getitem__(self, idx):
        if self.split.startswith('train'):
            ...
            sample = {'pose': self.poses[idx], 'img_idxs': idx, 'ts': torch.tensor([0]).int() }
...
class NSVFDataset_NGPA_lb(BaseDataset):
    def __init__(self, root_dir, split='train', downsample=1.0, **kwargs):
        super().__init__(root_dir, split, downsample)
    ...
    def read_meta(self, split):
        ...
        if split == 'test_traj': # BlendedMVS and TanksAndTemple
            ...
            if split == 'train':
                ...
                if min(self.id_train_final) >= self.img_per_appearance:
                    self.id_train_final[-1] = 0
            else:
            ...
            for img_id, id_train in enumerate(tqdm(self.id_train_final)):
                ...
                c2w[:, 3] /= 2*self.scale # to bound the scene inside [-0.5, 0.5]
                self.poses += [c2w]
                t = id_train//self.img_per_appearance
                # print("t[{}] = {}".format(id_train, t))
                img = read_image_ngpa(img_path, self.img_wh, split = split, t = t, test_img_gen = False)
...
class NSVFDataset_NGPA_CLNerf(BaseDataset):
    def __init__(self, root_dir, split='train', downsample=1.0, **kwargs):
        super().__init__(root_dir, split, downsample)
    ...
    def read_meta(self, split):
        ...
        if split == 'test_traj': # BlendedMVS and TanksAndTemple
            ...
            for img_id, id_train in enumerate(tqdm(self.id_train_final)):
                ...
                if self.rep_dir in img_path:
                    print("do not add color perturbation for {}".format(img_path))
                    img = read_image(img_path, self.img_wh, test_img_gen = True, img_id = id_train)
                else:
                    img = read_image_ngpa(img_path, self.img_wh, split = split_name, t = t, test_img_gen = True, img_id = id_train)
