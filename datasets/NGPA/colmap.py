...
# given all images' paths
# return the task id and the id of the test images
# pick 1/8 of the images from each task, at least 1 for each task
...
def name_to_task(img_paths):
    ...
    for i in range(len(task_list)):
        if task_list[i] > task_curr:
            ...
            test_ids.append(i)
        img_count += 1
    # print("tasks = {}; task_list = {}, test_ids = {}".format(tasks, task_list, test_ids))
    # exit()
...
class ColmapDataset_NGPA(BaseDataset):
    ...
    def read_meta(self, split, **kwargs):
        ...
        scale = max_far/8.0
        print("[test] near_far = {}/{}, scale = {}".format(min_near, max_far, scale))        
        self.poses[..., 3] /= scale
        self.pts3d /= scale

        # # scale = np.linalg.norm(self.poses[..., 3], axis=-1).min()
        # scale = np.linalg.norm(self.poses[..., 3], axis=-1).max()
        # # print("scale = {}/{}".format(scale, np.linalg.norm(self.poses[..., 3], axis=-1).max()))
        # self.poses[..., 3] /= scale
        # self.pts3d /= scale
        ...
        # train test split
        if split=='train':
            img_paths = [x for i, x in enumerate(img_paths) if i not in test_img_ids]
            self.poses = np.array([x for i, x in enumerate(self.poses) if i not in test_img_ids])
            task_ids = [x for i, x in enumerate(task_ids) if i not in test_img_ids]
        elif split=='test':
            img_paths = [x for i, x in enumerate(img_paths) if i in test_img_ids]
            self.poses = np.array([x for i, x in enumerate(self.poses) if i in test_img_ids])
    ...
    def __getitem__(self, idx):
        if self.split.startswith('train'):
            ...
            sample = {'pose': self.poses[idx], 'img_idxs': idx, 'ts':  torch.tensor([self.ts[idx].item()]).int() }
...
class ColmapDataset_NGPA_lb(BaseDataset):
    ...
    def read_meta(self, split, **kwargs):
        ...
        if split=='train':
            img_paths = [x for i, x in enumerate(img_paths) if i not in test_img_ids]
            self.poses = np.array([x for i, x in enumerate(self.poses) if i not in test_img_ids])
            self.task_ids = [x for i, x in enumerate(task_ids) if i not in test_img_ids]
        elif split=='test':
            img_paths = [x for i, x in enumerate(img_paths) if i in test_img_ids]
            self.poses = np.array([x for i, x in enumerate(self.poses) if i in test_img_ids])
            self.task_ids = [x for i, x in enumerate(task_ids) if i in test_img_ids]
        ...
        self.rays = torch.stack(self.rays) # (N_images, hw, ?)
        self.poses = torch.FloatTensor(self.poses[self.id_train_final]) # (N_images, 3, 4)
        self.ts = torch.tensor(self.ts).int()
...
class ColmapDataset_NGPA_CLNerf(BaseDataset):
    ...
    def read_meta(self, split, **kwargs):
        ...
        print("[split-{}] len(img_paths) = {}, id_train = {}".format(self.split, len(img_paths), self.id_train_final))
        # exit()
        print(f'Loading {len(img_paths)} {split} images ...')
        # for i, img_path in enumerate(tqdm(img_paths)):
        for img_id, id_train in enumerate(tqdm(self.id_train_final)):
        ...
        if self.split == 'test':
            print("[test ts]: self.ts = {}, self.task_ids = {}, len(ts) = {}, len(self.poses) = {}".format(self.ts, self.task_ids, self.ts.shape, len(self.poses)))
    ...
    def __getitem__(self, idx):
        if self.split.startswith('train'):
            ...
            sample = {'pose': self.poses[idx], 'ts': torch.tensor([self.ts[idx].item()]).int(), 'img_idxs': idx, 'fname': os.path.basename(self.img_paths[self.id_train_final[idx]]),'id_ori': self.id_train_final[idx],'task_id': self.task_ids[self.id_train_final[idx]]}
            if len(self.rays)>0: # if ground truth available
                rays = self.rays[idx]
                sample['rgb'] = rays[:, :3]
                if rays.shape[1] == 4: # HDR-NeRF data
        ...
        if self.split.startswith('train'):
            if self.task_curr == 0:
                ...
                sample = {'img_idxs': img_idxs, 
                        ...
                        'rgb': rays[:, :3], 
                        'ts': self.ts[img_idxs],
                        'is_rep': torch.cat((torch.ones(self.batch_size//3).int(), torch.zeros(self.batch_size-self.batch_size//3).int()))}
                if self.rays.shape[-1] == 4: # HDR-NeRF data
                    sample['exposure'] = rays[:, 3:]
