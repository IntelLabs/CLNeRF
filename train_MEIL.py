...
from losses import MEILNeRFLoss
...
class NeRFSystem(LightningModule):
    def __init__(self, hparams):
        ...
        self.loss = MEILNeRFLoss(lambda_distortion=self.hparams.distortion_loss_w)
    ...
    def setup(self, stage):
        ...
        kwargs = {'root_dir': self.hparams.root_dir,
                  ...
                  'rep_dir': f'results/MEIL/{self.hparams.dataset_name}/{self.hparams.exp_name}/rep',
    ...
    def training_step(self, batch, batch_nb, *args):
        ...
        # compute lambda_p
        task_ratio = float(self.global_step) / float(20000)
        lambda_p = 0.5*(math.cos(math.pi*(1+task_ratio))+1)
        loss_d = self.loss(results, batch, lambda_p)
    ...
    def on_validation_start(self):
        ...
        if not self.hparams.no_save_test:
            self.val_dir = f'results/MEIL/{self.hparams.dataset_name}/{self.hparams.exp_name}'
    ...
    def on_test_start(self):
        torch.cuda.empty_cache()
        self.rep_dir = f'results/MEIL/{self.hparams.dataset_name}/{self.hparams.exp_name}/rep'
