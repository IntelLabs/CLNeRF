...
class NeRFSystem(LightningModule):
    def __init__(self, hparams):
        ...
        # self.model = NGPGv2(scale=self.hparams.scale, vocab_size=self.hparams.vocab_size, rgb_act=rgb_act, dim_a = self.hparams.dim_a)
    ...
    def training_step(self, batch, batch_nb, *args):
        if self.global_step%self.update_interval == 0:
            if self.hparams.rep_size == 0:
                self.model.update_density_grid(0.01*MAX_SAMPLES/3**0.5,
                                            warmup=self.global_step<self.warmup_steps,
                                            erode=self.hparams.dataset_name=='colmap', last_only = True)
            else:
                self.model.update_density_grid(0.01*MAX_SAMPLES/3**0.5,
                                            warmup=self.global_step<self.warmup_steps,
...
if __name__ == '__main__':
    ...
    ckpt_cb = ModelCheckpoint(dirpath=f'ckpts/NGPGv2_lb/{hparams.dataset_name}/{hparams.exp_name}',
                              filename='{epoch:d}',
    ...
    logger = TensorBoardLogger(save_dir=f"logs/NGPGv2_lb/{hparams.dataset_name}",
    ...
    if not hparams.val_only: # save slimmed ckpt for the last epoch
        ckpt_ = \
            slim_ckpt(f'ckpts/NGPGv2_lb/{hparams.dataset_name}/{hparams.exp_name}/epoch={hparams.num_epochs-1}.ckpt',
                      save_poses=hparams.optimize_ext)
        torch.save(ckpt_, f'ckpts/NGPGv2_lb/{hparams.dataset_name}/{hparams.exp_name}/epoch={hparams.num_epochs-1}_slim.ckpt')
