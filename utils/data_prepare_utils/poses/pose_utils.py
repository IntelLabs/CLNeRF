...
def video2imgs(basedir, frame_rate = 10):
    # convert all videos under basedir to images in the same folder
    # get all video dirs
    imgdir = os.path.join(basedir, 'videos')
    videofiles = [os.path.join(imgdir, f) for f in sorted(os.listdir(imgdir)) if f.endswith('MOV') or f.endswith('mov') or f.endswith('mp4') or f.endswith('MP4') or f.endswith('avi') or f.endswith('AVI')]
    # get the path of each file
    for v_name in videofiles:
        print("converting {} to images with 10 frames per images".format(v_name))
        v_path = os.path.dirname(v_name)
        v_basename = Path(v_name).stem
        ...
        while success:
            ...
            if count % frame_rate == 0:
                write_dir = os.path.join(v_path.replace('videos', 'images'), v_basename)
                if not os.path.isdir(write_dir):
                    os.makedirs(write_dir, exist_ok = True)
                cv2.imwrite(os.path.join(write_dir, v_basename+'_{}.png'.format(count)), image)    
            count += 1            
            success, image = vidcap.read()
        print("converting finished")

def gen_poses(basedir, match_type, is_video=0, frame_rate=10, factors=None):
