...
def read_image_ngpa(img_path, img_wh, blend_a=True, split='train', t = 0, test_img_gen = False, img_id = 0):
    img = imageio.imread(img_path).astype(np.float32)/255.0
    ...
    if test_img_gen and split == 'train':
        print("[test after train]: t = {}, img_id= {}, img = {}/{}/{}".format(t, img_id, img.shape, img.min(), img.max()))
        # exit()
        # save the training image
        print("saving training image to {}".format(
...
def read_image_phototour(img_path, blend_a=True, test_img_gen = False, img_id = 0, downscale = 1, crop_region = 'full'):
    img = imageio.imread(img_path).astype(np.float32)/255.0
