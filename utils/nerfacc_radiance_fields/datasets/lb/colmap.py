...
def _load_colmap(root_fp: str, subject_id: str, split: str, factor: int = 1):
    ...
    # assert params is None, "Only support pinhole camera model. type = {}".format(type_)
    ...
    # print("image_names = {}, image_dir = {}".format(image_names, image_dir))
    image_paths = [
        os.path.join(image_dir, f) for f in image_names
    ]
    # get the task id
    task_ids, test_img_ids, train_img_ids = name_to_task(image_paths)
    # print("image_paths = {}, task_ids = {}. test_img_ids = {}".format(image_paths, task_ids, test_img_ids))
    ...
    pts3d = read_points3d_binary(os.path.join(data_dir, 'sparse/0/points3D.bin'))
    pts3d = np.array([pts3d[k].xyz for k in pts3d]) # (N, 3)
    
    # compute near far
    xyz_world_h = np.concatenate(
        [pts3d,
            np.ones((len(pts3d), 1))], -1)
    ...
    scale = max_far/1.25
    # print("shift = {}, scale = {}".format(shift, scale))

    # min_xyz, max_xyz = pts3d.min(axis = 0), pts3d.max(axis=0)
    # # print("min_xyz.shape = {}, max_xyz = {}".format(min_xyz.shape, max_xyz.shape))
    # shift = (max_xyz-min_xyz) / 2
    # scale = (max_xyz-min_xyz).max()/2*1.5*1.05 # rescale into AABB, discuss later 
    # print("shift = {}/{}/{}, scale = {}, camtoworlds.shape = {}/{}".format(min_xyz, max_xyz, shift, scale, camtoworlds.shape, camtoworlds[0]))
...
class SubjectLoader_lb(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.task_ids = (torch.from_numpy(self.task_ids).to(torch.uint8))[self.id_train_final]
    ...
    def split_tasks(self, num_img, task_number, task_split_method):
        ...
        if task_split_method == 'random':
            ...
            for j in range(task_number):
                ...
                # task_id.append()
            task_id += ([task_number-1] * (num_img- imgs_per_task * task_number)) 
        return task_id
