...
class NerfMLPG(nn.Module):
    def __init__(
        ...
        hidden_features = self.base.output_dim
        self.sigma_layer = DenseLayer(hidden_features + dim_g, 1)

        if condition_dim > 0:
            self.bottleneck_layer = DenseLayer(hidden_features, net_width)
            self.rgb_layer = MLP(
                input_dim=net_width + condition_dim + dim_a,
                output_dim=3,
                net_depth=net_depth_condition,
                net_width=net_width_condition,
                skip_layer=None,
            ...
            self.rgb_layer = DenseLayer(hidden_features + dim_a, 3)

    def query_density(self, x, f_g):
        x = self.base(x)
        raw_sigma = self.sigma_layer(torch.cat([x, f_g], dim=-1))
        return raw_sigma

    def forward(self, x, f_a, f_g, condition=None):
        x = self.base(x)
        raw_sigma = self.sigma_layer(torch.cat([x, f_g], dim=-1))
        if condition is not None:
            if condition.shape[:-1] != x.shape[:-1]:
            ...
            bottleneck = self.bottleneck_layer(x)
            x = torch.cat([bottleneck, condition], dim=-1)
        raw_rgb = self.rgb_layer(torch.cat([x, f_a], dim=-1))
        return raw_rgb, raw_sigma


class VanillaNeRFRadianceFieldG(nn.Module):
    def __init__(
        self,
        net_depth: int = 8,  # The depth of the MLP.
        net_width: int = 256,  # The width of the MLP.
        ...
        print("[init geometric nerf]: vocab_size = {}, dim_a = {}, dim_g = {}".format(vocab_size, dim_a, dim_g))
    ...
    def query_density(self, x, embed_id):
        # print("x = {}/{}, embed_id = {}/{}".format(x, x.shape, embed_id, embed_id.shape))
        x = self.posi_encoder(x)
        sigma = self.mlp.query_density(x, self.geometry_embed(embed_id.flatten()))
        return F.relu(sigma)

    def forward(self, x, embed_id, condition=None):
        x = self.posi_encoder(x)
        if condition is not None:
            condition = self.view_encoder(condition)
        rgb, sigma = self.mlp(x, self.appearance_embed(embed_id.flatten()), self.geometry_embed(embed_id.flatten()), condition=condition)
        return torch.sigmoid(rgb), F.relu(sigma)




class NerfMLPA(nn.Module):
    def __init__(
        self,
        input_dim: int,  # The number of input tensor channels.
        condition_dim: int,  # The number of condition tensor channels.
    ...
    def forward(self, x, f_a, condition=None):
        x = self.base(x)
        raw_sigma = self.sigma_layer(x)
        if condition is not None:
            if condition.shape[:-1] != x.shape[:-1]:
...
class VanillaNeRFRadianceFieldA(nn.Module):
    def __init__(
        self,
        net_depth: int = 8,  # The depth of the MLP.
        net_width: int = 256,  # The width of the MLP.
        ...
        # appearance embedding
        self.appearance_embed = torch.nn.Embedding(vocab_size, dim_a)
        print("[init geometric nerf]: vocab_size = {}, dim_a = {}".format(vocab_size, dim_a))
    ...
    def forward(self, x, embed_id, condition=None):
        ...
        if condition is not None:
            condition = self.view_encoder(condition)
        rgb, sigma = self.mlp(x, self.appearance_embed(embed_id.flatten()), condition=condition)
        return torch.sigmoid(rgb), F.relu(sigma)
