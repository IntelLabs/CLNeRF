...
std::vector<torch::Tensor> raymarching_train_ngpa(
    const torch::Tensor rays_o,
    const torch::Tensor rays_d,
    ...
    CHECK_INPUT(rays_o);
    CHECK_INPUT(rays_d);
    CHECK_INPUT(img_idxs);
    CHECK_INPUT(hits_t);
    CHECK_INPUT(density_bitfield);
    CHECK_INPUT(noise);

    return raymarching_train_ngpa_cu(
        rays_o, rays_d, img_idxs, hits_t, density_bitfield, cascades,
        scale, exp_step_factor, noise, grid_size, max_samples);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){
    ...
    m.def("raymarching_train_ngpa", &raymarching_train_ngpa);
