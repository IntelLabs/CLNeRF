...
from .custom_functions import \
    RayAABBIntersector, RayMarcher, RayMarcher_NGPA, VolumeRenderer
...
def render(model, rays_o, rays_d, img_idxs, **kwargs):
    """
    Render rays by
    ...
    results = render_func(model, rays_o, rays_d, img_idxs, hits_t, **kwargs)
...
@torch.no_grad()
def __render_rays_test(model, rays_o, rays_d, img_idxs, hits_t, **kwargs):
    """
    Render rays by
    ...
    while samples < kwargs.get('max_samples', MAX_SAMPLES):
        ...
        sigmas[valid_mask], _rgbs = model(xyzs[valid_mask], dirs[valid_mask], img_id = img_idxs[0].item() * torch.ones((xyzs.size()[0], 1), device = img_idxs.device).int()[valid_mask], embed_test = None, **kwargs)
...
def __render_rays_train(model, rays_o, rays_d, img_idxs, hits_t, **kwargs):
    """
    Render rays by
    ...
    (rays_a, xyzs, dirs,
    results['deltas'], results['ts'], embed_idxs, results['rm_samples']) = \
        RayMarcher_NGPA.apply(
            rays_o, rays_d, img_idxs, hits_t[:, 0], model.density_bitfield,
    ...
    # print("[test render] embed_idxs = {}".format(embed_idxs))
    sigmas, rgbs = model(xyzs, dirs, img_id = embed_idxs,  embed_test = None, **kwargs)
